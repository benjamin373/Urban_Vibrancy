"""
This script analyzes urban vibrancy in a given city by aggregating various data sources, 
including OpenStreetMap (OSM), Open-Meteo weather data, and Google Street View pedestrian detections. 
The results are visualized using a combination of folium maps and Matplotlib.

### Functionality:

1. **User Input and Configuration:**
   - The user specifies a city, date, and time for the analysis.
   - The script automatically determines the season and assigns multipliers for urban vibrancy factors 
     (trees, pathways, and points of interest).

2. **Data Retrieval:**
   - **City Coordinates:** Queries OSM for the latitude and longitude of the city center.
   - **Weather Data:** Retrieves hourly weather conditions for the specified date and time from Open-Meteo.
   - **OSM Data Extraction:** Uses the Overpass API to obtain:
     - Footways (pedestrian paths)
     - Trees (green spaces)
     - Points of Interest (POIs such as cafes, bus stops, shops, etc.)
   - **Street View Pedestrian Data:** Loads preprocessed people detection data from a GeoJSON file, 
     based on YOLO11 object detection applied to Google Street View images.

3. **Data Processing and Aggregation:**
   - Converts OSM elements into `GeoDataFrames`.
   - Assigns each data point to an H3 hexagonal grid cell.
   - Aggregates footways, trees, POIs, and pedestrian counts per hexagon.
   - Applies seasonal and weather-based multipliers to normalize urban vibrancy values.

4. **Visualization:**
   - **Folium Interactive Map:** Generates a web-based map displaying:
     - Footways, Trees, and POIs in separate layers.
     - Urban vibrancy index as a heatmap using H3 hexagons.
     - Locations without data are marked in gray.
   - **Matplotlib Static Map:** 
     - Plots the urban vibrancy density in a hexagonal grid.
     - Includes north arrow, scale bar, and data sources.
     - Exports the final visualization in GeoJSON format for GIS applications.

5. **Output:**
   - A folium-based interactive HTML map (`*_combined_map_footways.html`).
   - Processed GeoJSON files (`combined_density_res11.geojson`).
   - A Matplotlib-based static urban vibrancy map.

### Requirements:
- Preprocessed Street View pedestrian count data in GeoJSON format.
"""
